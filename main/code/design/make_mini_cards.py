...
def saveMiniCard(index:int=0, path_for_save:str="main/assets/images/mini_cards") -> None:
    ...
    if row_list[-1] == "CH":
        ...
        im_card.paste(im_flag, (350, 1225))
    else:
        im_flag = im_flag.resize((160, 100))
        im_card.paste(im_flag, (320, 1225))
        
    # LEAGUE
    im_league = im_league.resize((200, 200))
    league_ar = np.array(im_league)
    league_mk = (league_ar[:, :, 3] > 0)
    im_card.paste(im_league, (550, 1175), Image.fromarray(league_mk))
    ...
    if team_name == "unemployed":
        ...
        team_mk = (team_ar[:, :, 3] > 0)
        im_card.paste(im_team, (800, 1200), Image.fromarray(team_mk))
    else:
        im_team = im_team.resize((180, 180))
        team_ar = np.array(im_team)
        team_mk = (team_ar[:, :, 3] > 0)
        im_card.paste(im_team, (800, 1175), Image.fromarray(team_mk))
